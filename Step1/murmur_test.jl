...
@testset "log_sum_body" begin
    ...
    @test abs(log_sum_body(1, 0.51, 9)-0.0299096375126409) < 1e-17
    @test abs(log_sum_body(1, 0.51, 9)-0.0299096375126409)/log_sum_body(1, 0.51, 9) < 1e-15
    @test abs(log_sum_body(2, 0.51, 9)-0.001655951531561063859516483) < 1e-18
    @test abs(log_sum_body(2, 0.51, 9)-0.001655951531561063859516483)/log_sum_body(2, 0.51, 9) < 1e-15
    @test abs(log_sum_body(3, 0.51, 9)-0.000222745391052210891629437964054484) < 1e-19
    @test abs(log_sum_body(3, 0.51, 9)-0.000222745391052210891629437964054484)/log_sum_body(3, 0.51, 9) < 1e-15
    @test abs(log_sum_body(4, 0.51, 9)-0.0000802217525874537526203420827542224126) < 1e-19
    @test abs(log_sum_body(4, 0.51, 9)-0.0000802217525874537526203420827542224126)/log_sum_body(4, 0.51, 9) < 1e-15
    @test abs(log_sum_body(5, 0.51, 9)-0.0000802217525874537526203420827542224126) < 1e-19
    @test abs(log_sum_body(5, 0.51, 9)-0.0000802217525874537526203420827542224126)/log_sum_body(5, 0.51, 9) < 1e-15
end
